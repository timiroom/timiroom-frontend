...
/* ── Stat Card ── */
function StatCard({ icon, label, value, unit="", sub, color="#A78BFA", delay=0 }) {
    ...
    <div className="db-card" style={{
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
        <div style={{
          ...
          background:`rgba(${color === "#A78BFA" ? "167,139,250" : color === "#10B981" ? "16,185,129" : color === "#3B82F6" ? "59,130,246" : "245,158,11"},.15)`,
          display:"flex", alignItems:"center", justifyContent:"center", fontSize:20,
          boxShadow:`0 0 14px rgba(${color === "#A78BFA" ? "167,139,250" : "139,92,246"},.2)`,
        }}>{icon}</div>
        <span style={{ fontSize:11, color:"var(--db-text-muted)", background:"var(--db-bg-elevated)", padding:"3px 8px", borderRadius:100 }}>24h</span>
      </div>
      ...
      <div style={{ fontSize:13, color:"var(--db-text-secondary)", marginTop:4 }}>{label}</div>
      {sub && <div style={{ fontSize:11, color:"var(--db-text-muted)", marginTop:8, paddingTop:8, borderTop:"1px solid var(--db-border)" }}>{sub}</div>}
    </div>
  );
}
    ...
    <div className="db-card" style={{ padding:"24px", display:"flex", alignItems:"center", gap:24 }}>
      <div style={{ position:"relative", width:110, height:110, flexShrink:0 }}>
        <svg width="110" height="110" viewBox="0 0 110 110">
          <circle cx="55" cy="55" r={r} fill="none" stroke="var(--db-bg-elevated)" strokeWidth="8"/>
          <circle
        ...
        <div style={{
          ...
          <div style={{ fontSize:10, color:"var(--db-text-muted)" }}>/ 100</div>
        </div>
      </div>
      <div style={{ flex:1 }}>
        <div style={{ fontSize:15, fontWeight:700, marginBottom:4 }}>정합성 스코어</div>
        <div style={{ fontSize:12, color:"var(--db-text-muted)", marginBottom:16 }}>현재 이커머스 리뉴얼 프로젝트</div>
        {[
          { label:"API 명세", pct:88, color:"#A78BFA" },
          { label:"DB 스키마", pct:72, color:"#3B82F6" },
          { label:"QA 커버리지", pct:65, color:"#10B981" },
        ].map((item) => (
          <div key={item.label} style={{ marginBottom:8 }}>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"var(--db-text-secondary)", marginBottom:4 }}>
    ...
    <div className="db-card" style={{ padding:"20px 24px" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
        {ACTIVITIES.map((a, i) => {
          ...
          const bgs    = { green:"var(--db-green-bg)", orange:"var(--db-orange-bg)", purple:"rgba(139,92,246,.12)", blue:"var(--db-blue-bg)" };
          return (
            <div key={i} style={{
            ...
            >
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:13, fontWeight:600, color:"var(--db-text-primary)" }}>{a.text}</div>
                <div style={{ fontSize:11, color:"var(--db-text-muted)", marginTop:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{a.sub}</div>
              </div>
              <div style={{ fontSize:11, color:"var(--db-text-muted)", flexShrink:0 }}>{a.time}</div>
            </div>
          );
        })}
      </div>
...
function AgentStatus() {
  const statusStyles = {
    ...
    idle:    { color:"var(--db-text-muted)", bg:"var(--db-bg-elevated)", label:"대기",   dot:false },
          ...
          return (
            <div key={ag.name} style={{
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:11, color:"var(--db-text-muted)" }}>{ag.role}</div>
              </div>
              <div style={{
...
export function OverviewPanel() {
  const stats = [
    { icon:"📁", label:"활성 프로젝트", value:3,  unit:"개", sub:"↑ 1 이번 주 신규", color:"#A78BFA", delay:0   },
