    ...
    <section style={{ flex: 1, overflowY: "auto", padding: 28, background: "linear-gradient(180deg, var(--db-bg-primary) 0%, var(--bg) 100%)" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 22, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1)", letterSpacing: "-.02em" }}>Issues</div>
            <div style={{ fontSize: 13, color: "var(--text-3)", marginTop: 6 }}>연결된 GitHub 레포의 진행 중·완료 이슈를 한 곳에서 봅니다.</div>
          </div>
          {canManage && <button type="button" onClick={() => setComposerOpen((open) => !open)} disabled={repos.length === 0} style={{ padding: "9px 14px", border: "none", borderRadius: 10, background: "var(--text-1)", color: "var(--bg)", fontSize: 13, fontWeight: 700, cursor: repos.length ? "pointer" : "not-allowed", fontFamily: "inherit" }}>{composerOpen ? "취소" : "이슈 만들기"}</button>}
        </div>
